Subroutine Set_Initial_Conditions(f,S,T,D,fm,lat)
      ...
      do i = 1,im 
        ...
        if ( fm( i, j ) .ne. 1 ) then ! Land cell.
          f( i, j, k, : ) = -9999.
            ...
            !DIC
            temp = -36.89*S(i,j,k) + 3.06*D(i,j,k) + 0.81*S(i,j,k)*S(i,j,k) - 0.04*D(i,j,k)*D(i,j,k) + 2438.05
            ...
            ! y = -(0.01586)x + 2.9573
            call depth2press(D(i,j,k), lat(j), pdbar, 1)
