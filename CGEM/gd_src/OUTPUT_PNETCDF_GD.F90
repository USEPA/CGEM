...
CONTAINS
  ...
  SUBROUTINE CREATE_FILE( NAME, IM, JM, KM, NSTEP, VARIABLES,    &
   ...
   ! Locals:
    ...
    ERR = NFMPI_CREATE( MPI_COMM_SELF, trim(NAME), &
                        IOR( NF_CLOBBER, NF_64BIT_OFFSET ), INFO, FILE_ID )
...
!GoMDOM Parameters
    ...
    ERR = NFMPI_ENDDEF( FILE_ID )
    CALL CHKERR( ERR, 'create NetCDF output header' )
    ERR = NFMPI_BEGIN_INDEP_DATA( FILE_ID )
    ...
    ERR = NFMPI_PUT_VAR_REAL( FILE_ID, RLON_VAR, RLON_COPY )
    CALL CHKERR( ERR, 'write output variable rlon' )

    ERR = NFMPI_PUT_VAR_REAL( FILE_ID, RLAT_VAR, RLAT )
    CALL CHKERR( ERR, 'write output variable rlat' )

    ! Write h
    ERR = NFMPI_PUT_VAR_REAL( FILE_ID, H_VAR, H )
    CALL CHKERR( ERR, 'write output variable h' )

    ! Write fm in 3D

    ERR = NFMPI_PUT_VAR_REAL( FILE_ID, FM_VAR, FM )
    CALL CHKERR( ERR, 'write output variable fm' )

    ERR = NFMPI_PUT_VAR_REAL( FILE_ID, DZ_VAR, DZ )
    CALL CHKERR( ERR, 'write output variable dz' )

    ERR = NFMPI_PUT_VAR_REAL( FILE_ID, AREA_VAR, AREA )
  ...
  SUBROUTINE OPEN_FILE( NAME, VARIABLES, EXTRA_VARIABLES, FIRST_TIMESTEP )
    ...
    ERR = NFMPI_OPEN( MPI_COMM_WORLD, trim(NAME), &
    ...
    ERR = NFMPI_INQ_VARID( FILE_ID, 'time', TIME_VAR )
    ...
    DO VARIABLE = 1, VARIABLES
      ...
      IF ( WRITE_VARIABLE( VARIABLE ) ) THEN
        ERR = NFMPI_INQ_VARID( FILE_ID, TRIM(VARIABLE_NAMES( VARIABLE )), &
                               F_VAR( VARIABLE ) )
        CALL CHKERR( ERR, 'inquire NetCDF variable ID ' )
      END IF
    END DO
    ...
    DO VARIABLE = 1, EXTRA_VARIABLES
      ...
      IF ( WRITE_EXTRA_VARIABLE( VARIABLE ) ) THEN
        ERR = NFMPI_INQ_VARID( FILE_ID, TRIM(EXTRA_VARIABLE_NAMES( VARIABLE )), &
                               EXTRA_VAR( VARIABLE ) )
        CALL CHKERR( ERR, 'inquire NetCDF variable ID ' )
      END IF
    END DO
  ...
  SUBROUTINE WRITE_DATA( ISTART, IM, JSTART, JM, ZSTART, KM, VARIABLES, TIMESTEP, F ) 
    USE OUTPUT 
    IMPLICIT NONE
    ...
    INTEGER,DIMENSION(VARIABLES + 1):: REQUESTS, STATUSES
    ...
    ERR = NFMPI_IPUT_VARA_DOUBLE( FILE_ID, TIME_VAR, STARTS, COUNTS,SECONDS,REQUESTS(1)) 
    ...
    DO VARIABLE = 1, VARIABLES
      ...
      IF ( WRITE_VARIABLE( VARIABLE ) ) THEN
        ...
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, F_VAR( VARIABLE ), &
    ...
    ERR = NFMPI_WAIT_ALL( FILE_ID, REQUEST_COUNT, REQUESTS, STATUSES )
  ...
  !
  SUBROUTINE WRITE_EXTRA_DATA( ISTART, IM, JSTART, JM, ZSTART, KM, EXTRA_VARIABLES, TIMESTEP) 
    USE OUTPUT 
    USE MASS_BALANCE_GD
...
&       PFG, NFG, IFG, TFG, OUTPUT_PAR
    ...
    ! Locals:
    INTEGER(MPI_OFFSET_KIND):: STARTS( 4 ), COUNTS( 4 )
    INTEGER(MPI_OFFSET_KIND):: STARTS3(3),COUNTS3(3)
    ...
    STARTS3( 1 ) = ISTART
    STARTS3( 2 ) = JSTART
    STARTS3( 3 ) = FILE_TIMESTEP + 1 ! NetCDF follows FORTRAN 1-based convention.
    COUNTS3( 1 ) = IM
    COUNTS3( 2 ) = JM
    COUNTS3( 3 ) = 1
    ...
    IF ( WRITE_EXTRA_VARIABLE( VARIABLE ) ) THEN
      REQUEST_COUNT = REQUEST_COUNT + 1
      ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                  STARTS, COUNTS, &
    ...
    END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                 SUM_DOCPRD( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    SUM_DOCMET( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    SUM_DOCZOO( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    PD_AVG( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    PG_AVG( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    NITDO2( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    DOMETD_ARR( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    DOMETG_ARR( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    DOPREDD_ARR( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    DOPREDG_ARR( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    DOZOO_ARR( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    DOMNLDOC_ARR( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    PFD( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    SFD( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    NFD( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    IFD( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    TFD( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    PFG( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    NFG( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    IFG( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    TFG( 1, 1, 1 ),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
      ...
      REQUEST_COUNT = REQUEST_COUNT + 1
        ERR = NFMPI_IPUT_VARA_REAL( FILE_ID, EXTRA_VAR( VARIABLE ), &
                                    STARTS, COUNTS, &
                                    OUTPUT_PAR( 1, 1, 1),REQUESTS(REQUEST_COUNT))
        CALL CHKERR( ERR, 'write output variable  ' &
                     // EXTRA_VARIABLE_NAMES( VARIABLE ) )
      END IF
