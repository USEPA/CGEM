...
CONTAINS
  ...
  SUBROUTINE init_info_from_type(info)
    ...
    info%dimLocs = -1
    do i = 1, info%ndims
      if(info%dimNames(i).eq.'X' .or. info%dimNames(i).eq.'x' .or. info%dimNames(i).eq.'I' .or. info%dimNames(i).eq.'i' .or. &
         info%dimNames(i).eq.'Number_Rivers' .or. info%dimNames(i).eq.'Number_BoundaryCells') then
         info%dimLocs(1) = i
    ...
    info%varLocs = -1
    do i = 1, info%nvars
      if(info%varNames(i) .eq. 'X' .or. info%varNames(i) .eq. 'x' .or. info%varNames(i) .eq. 'I' .or. info%varNames(i) .eq. 'i' .or. &
         info%varNames(i).eq.'Number_Rivers' .or. info%varNames(i).eq.'Number_BoundaryCells') then
         info%varLocs(1) = i
