      Subroutine Flux(f,T,S,Esed,CBODW,pH,wsm,fm,dz,Wsp,istep)
...
!--------------------------------------------------------------
            ...
            !---------------------------------------------------------------
             if(Wsp(i,j).lt.3.6) then
               Vtrans        = AMAX1((5.9 * (0.17*Wsp(i,j))         &
               &              * Sc**(-2./3.) / SDay), 0.)
             else if(Wsp(i,j).le.13.) then
               Vtrans        = AMAX1((5.9 *(2.85*Wsp(i,j) - 9.65 )    &
               &              / SQRT(Sc) / SDay), 0.)
             else
               Vtrans        = AMAX1((5.9 *(5.9*Wsp(i,j) - 49.3 )    &
               &              / SQRT(Sc) / SDay), 0.)
             endif
...
!---------------using mocsy:------------------------------------
  kw660(1) = 0.01/3600.*0.31*Wsp(i,j)*Wsp(i,j) !*schmidtnumberterm
  pCO2_in(1) = pCO2
...
if(Which_Fluxes(iSDM).eq.1) then
   ...
   write(6,*) "SDM Flux has not been tested, please turn off and rerun."
   stop

!Sediment Diagenesis Model
        if(init.eq.1.or.mod(istep,288).eq.0) then  !Call every day, 288 timestep, assumes timestep = 5 min
           call Sediment_Diagenesis_Flux(A,f(i,j,nz,:),T(i,j,nz),S(i,j,nz),pH(i,j,nz),sedflux(i,j,:),i,j,YY_init,ppH_init)
        endif
...
!O2 Exchange
     ...
     & dz(i,j,nz)*dT/SDay,0.)
!OM1 Exchange
               f(i,j,nz,iOM1_bc) = AMAX1(f(i,j,nz,iOM1_bc) - sedflux(i,j,iOM1_bc)/ &
     & dz(i,j,nz)*dT/SDay,0.)
!OM2 Exchange
