...
!----------------------------------------------------------------
      ...
      case (2) ! Use Cloern Chl:C ratio
         Chla_tot_k = Chla_Cloern(A_k, Qn_k, Qp_k, Si_k, T_k, aDailyRad_k,Chl_C_k)
...
!-------------------------------------------------------------------------      
   ...
   !Monod Equations
     Ntotal       = NO3 + NH4 
     monodN(isp)  = AMAX1((Ntotal/(Ntotal+Kn(isp))),0.) 
     monodP(isp)  = AMAX1((PO4/(PO4+Kp(isp))),0.)      
     monodSi(isp) = AMAX1((Si/(Si+Ksi(isp))),0.)
...
!-----------------------------------------------------------------------
      ...
      !write(6,*) "C ZgrazA_tot*dTd",ZgrazA_tot(isp)*dTd*Qc(isp)
...
!---------------------------------------------------------------------
      ...
      !write(6,*) "Z1:ZgrazA,Carbon from grazing",ZgrazA(isp,1),ZgrazC(1)
      !write(6,*) "Z2:ZgrazA,Carbon from grazing",ZgrazA(isp,2),ZgrazC(2)
      !write(6,*) "Before calc, Carbon in A=",A_k(isp,k)*Qc(isp) 
      !write(6,*)
      !write(6,*) "Z1:ZgrazA,PO4 from grazing",ZgrazA(isp,1),ZgrazP(1)
      !write(6,*) "Z2:ZgrazA,PO4 from grazing",ZgrazA(isp,2),ZgrazP(2)
      !write(6,*) "Before calc, PO4 in A=",A_k(isp,k)*Qp


!---------------------------------------------------------
!-A; Phytoplankton number density (cells/m3);
!---------------------------------------------------------
      !A_k(isp,k)         = AMAX1(f(i,j,k,iA(isp))                              &
      !& + ( Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp) )*dTd,1.e-8)       
      !write(6,*) "A",istep,Agrow*dTd*Qp, -Aresp*dTd*Qp, - ZgrazA_tot(isp)*dTd*Qp, - Amort(isp)*dTd*Qp

      Diff_A = f(i,j,k,iA(isp))                              &
      & + ( Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp) )*dTd
      if(Diff_A.lt.0) then 
       SUM_DA = SUM_DA + Diff_A
       !write(6,*) "Loss:", SUM_DA*Qc(isp),SUM_DA*Qn,SUM_DA*Qp 
       !write(6,*) "f,grow,resp,graz,mort",f(i,j,k,iA(isp))*Qc(isp),Agrow*dTd*Qc(isp), &
      !& -Aresp*dTd*Qc(isp),-ZgrazA_tot(isp)*dTd*Qc(isp),-Amort(isp)*dTd*Qc(isp)
      endif
 
      !ff(i,j,k,iA(isp)) = A_k(isp,k) 
      ff(i,j,k,iA(isp)) = AMAX1(f(i,j,k,iA(isp))                              &
      & + ( Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp) )*dTd,1.e-18)
!      & + ( Agrow - ZgrazA_tot(isp) - Amort(isp) )*dTd,1.e-8) 
     !write(6,*) "After calc, Carbon in A=",A_k(isp,k)*Qc(isp)
     write(6,*) "Carbon in Grow,resp,graz,mort",Agrow*Qc(isp)*dTd,-Aresp*Qc(isp)*dTd,-ZgrazA_tot(isp)*Qc(isp)*dTd,&
&-Amort(isp)*Qc(isp)*dTd
     write(6,*) "Carbon in total, total*dT",(Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp))*Qc(isp),&
     & ( Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp) )*dTd*Qc(isp)
     !write(6,*) "After calc, PO4 in A=",A_k(isp,k)*Qp
     !write(6,*) "PO4 in Grow,resp,graz,mort",Agrow*Qp,-Aresp*Qp,-ZgrazA_tot(isp)*Qp,-Amort(isp)*Qp
     !write(6,*) "PO4 in total, total*dT",(Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp))*Qp,&
     !& ( Agrow - Aresp - ZgrazA_tot(isp) - Amort(isp) )*dTd*Qp
...
!       &               + (vP - Qp*uA - AexudP_A(isp))*dTd
     ! write(6,*) "Qp",istep,QminP(isp),f(i,j,k,iQp(isp)),vP*dTd,-Qp*uA*dTd,-AexudP_A(isp)*dTd
...
! Zooplankton growth is modified by a temperature adjustment factor
    !TEST  Zgrow(:) = Tadj(nospA+1:nospA+nospZ)*Zgrow(:)

! Zooplankton respiration based on growth and basal metabolism, both modified by a temperature adjustment factor 
   !TEST   Zresp(:) = (Zgrow(:)*Zrespg(:) + Tadj(nospA+1:nospA+nospZ)*Z(:)*Zrespb(:))  !Zooplankton respiration (indv./m3/d)
...
!---------------------------------------------------------
      ...
      &      + (Zgrow(:) - Zresp(:) - Zmort(:))*dTd, 1.e-18)
      write(6,*) "C in Z(1)",istep,Zgrow(1)*ZQc(1)*dTd,-Zresp(1)*ZQc(1)*dTd,-Zmort(1)*ZQc(1)*dTd,&
&"ToT",(Zgrow(1) - Zresp(1) - Zmort(1))*dTd*ZQc(1)
      write(6,*) "C in Z(2)",istep,Zgrow(2)*ZQc(2)*dTd,-Zresp(2)*ZQc(2)*dTd,-Zmort(2)*ZQc(2)*dTd,&
&"ToT",(Zgrow(2) - Zresp(2) - Zmort(2))*dTd*ZQc(2)
      write(6,*) "Total change in Z",(Zgrow(1) - Zresp(1) - Zmort(1))*dTd*ZQc(1) + &
&     (Zgrow(2) - Zresp(2) - Zmort(2))*dTd*ZQc(2)
!------------------------------------------------------------------------
...
!-- Organic Matter from dead phytoplankton --------------------------
                                             ! Particulate
    OM1_CA   = SUM(Amort*(QminN/Qn_k(:,k))*Qc)        ! (mmol-C/m3/d)
    OM1_NA   = SUM(Amort*QminN)                       ! (mmol-N/m3/d)
    OM1_PA   = SUM(Amort*(QminN/Qn_k(:,k))*Qp_k(:,k))      ! (mmol-P/m3/d)
                                             ! Dissolved
    OM2_CA    = SUM(Amort*((Qn_k(:,k)-QminN)/Qn_k(:,k))*Qc)   ! (mmol-C/m3/d)
    OM2_NA    = SUM(Amort*(Qn_k(:,k)-QminN))                  ! (mmol-N/m3/d)
    OM2_PA    = SUM(Amort*((Qn_k(:,k)-QminN)/Qn_k(:,k))*Qp_k(:,k)) ! (mmol-P/m3/d)

   !This calculates the cumulative stoichiometry ratios for OM1_A
   if(OM1_CA.ne.0) then
    stoich_x1A = (OM1_CA + OM1_A) / (OM1_PA + (1/s_x1A(i,j,k))*OM1_A) ! C/P
    stoich_y1A = (OM1_NA + (s_y1A(i,j,k)/s_x1A(i,j,k))*OM1_A) / (OM1_PA + (1/s_x1A(i,j,k))*OM1_A) !N/P
    stoich_z1A = 1.
   else
   ...
   if(OM2_CA.ne.0) then
    stoich_x2A = (OM2_CA + OM2_A) / (OM2_PA + (1/s_x2A(i,j,k))*OM2_A) ! C/P
    stoich_y2A = (OM2_NA + (s_y2A(i,j,k)/s_x2A(i,j,k))*OM2_A) / (OM2_PA + (1/s_x2A(i,j,k))*OM2_A) !N/P
    stoich_z2A = 1.
   else
...
!-- Organic Matter from fecal pellets ---------------------------------
    OM1_Ratio = SUM( (QminN/Qn_k(:,k))*A_k(:,k))/SUM(A_k(:,k))
    OM2_Ratio = SUM( (Qn_k(:,k)-QminN)/Qn_k(:,k)*A_k(:,k))/SUM(A_k(:,k))
    write(6,*) "OM_Ratio",OM1_Ratio,OM2_Ratio
                                                                  ! Particulate
    ...
    write(6,*) "istep,Z*ZQc,ZegC,ZunC,ZmortC_tot,OM_Ratio*ZslopC_tot"
    write(6,*) "C in Z1:", istep,Z(1)*ZQc(1),ZegC(1),ZunC(1),ZmortC_tot,OM1_Ratio*ZslopC_tot,&
&         "Tot=",ZegC(1) + ZunC(1) + ZmortC_tot + OM1_Ratio*ZslopC_tot
    write(6,*) "C in Z2:", istep,Z(2)*ZQc(2),ZegC(2),ZunC(2),OM2_Ratio*ZslopC_tot,&
&         "Tot=",ZegC(2) + ZunC(2) + OM2_Ratio*ZslopC_tot
    !write(6,*) "istep,A*Qp,Z,Z*ZQp,ZegP,ZunP,ZmortP_tot,OM_Ratio*ZslopP_tot"
    !write(6,*) "Z1:", istep,A_k(1,k)*Qp,Z(1),Z(1)*ZQp(1),ZegP(1),ZunP(1),ZmortP_tot,OM1_Ratio*ZslopP_tot
    !write(6,*) "Z2:", istep,A_k(1,k)*Qp,Z(2),Z(2)*ZQp(2),ZegP(2),ZunP(2),ZmortP_tot,OM2_Ratio*ZslopP_tot

   !This calculates the cumulative stoichiometry ratios for OM1_Z
   if(OM1_CZ.ne.0) then
    stoich_x1Z = (OM1_CZ + OM1_Z) / (OM1_PZ + (1./s_x1Z(i,j,k))*OM1_Z) ! C/P
    stoich_y1Z = (OM1_NZ + (s_y1Z(i,j,k)/s_x1Z(i,j,k))*OM1_Z) / (OM1_PZ + (1./s_x1Z(i,j,k))*OM1_Z) !N/P
    stoich_z1Z = 1.
   else
   ...
   if(OM2_CZ.ne.0) then
    stoich_x2Z = (OM2_CZ + OM2_Z) / (OM2_PZ + (1./s_x2Z(i,j,k))*OM2_Z) !  C/P
    stoich_y2Z = (OM2_NZ + (s_y2Z(i,j,k)/s_x2Z(i,j,k))*OM2_Z) / (OM2_PZ + (1./s_x2Z(i,j,k))*OM2_Z) !N/P
    stoich_z2Z = 1.
   else
...
! Nitrification
!--------------------------------------------------------------
        call Nitrification( AMAX1(O2,0.), NH4, k11, T_k(k), R_11 )
!        R_11 = R_11  * one_d_365
...
!--------------------------------------------------------------
        call reaction( OM1_A+OM1_CA, OM2_A+OM2_CA, AMAX1(O2,0.), NO3, KG1, KG2, KO2, KstarO2, KNO3,               &
     &  stoich_x1A, stoich_y1A, stoich_z1A, stoich_x2A, stoich_y2A, stoich_z2A, T_k(k), RC )
...
!--------------------------------------------------------------
        call reaction( OM1_Z+OM1_CZ, OM2_Z+OM2_CZ, AMAX1(O2,0.), NO3, KG1, KG2, KO2, KstarO2, KNO3,               &
     &  stoich_x1Z, stoich_y1Z, stoich_z1Z, stoich_x2Z, stoich_y2Z, stoich_z2Z, T_k(k), RC )
        ...
        !write(6,*) "OM_Z,ROM_Z",istep,OM1_Z,ROM1_Z,OM2_Z,ROM2_Z,O2,NO3 

!------------------------------------------------------------
! Particulate and Dissolved riverine OM, rate of remineralization 
!------------------------------------------------------------
        call reaction( OM1_R, OM2_R, AMAX1(O2,0.), NO3, KG1_R, KG2_R, KO2, KstarO2, KNO3,               &
...
!------------------------------------------------------------
        call reaction( OM1_BC, OM2_BC, AMAX1(O2,0.), NO3, KG1_BC, KG2_BC, KO2, KstarO2, KNO3,               &
...
! Sum remineralization terms from dead phytoplankton, fecal pellets, and riverine particulate
  ...
  write(6,*) "Pre-RNO3,R_11, RALK",RNO3-R_11,R_11,RALK
       !Save for netCDF
       RNO3_ijk(i,j,k) = RNO3_ijk(i,j,k) + (2*RN2)*dTd
! Save RO2 as CBODW
  CBODW(i,j) = RO2 !The last time this happens, k=nz, so will be the bottom
!--------------------------------------------------------------------
    write(6,*) "RDIC,RDIC_BC,OM1BC,OM2BC,OMBC",RDIC*dTd,RDIC_BC*dTd, ROM1_BC*dTd, ROM2_BC*dTd,(ROM1_BC+ROM2_BC)*dTd
...
!-------------------------------     
       ...
       write(6,*) istep,"NO3:",f(i,j,k,iNO3),RNO3*dTd,- AupN*NO3/(NO3+NH4)*dTd 
!--------------------------------
!-NH4; Ammonium (mmol-N/m3)
!--------------------------------
       ff(i,j,k,iNH4) = AMAX1(f(i,j,k,iNH4)                            &
       & + ( RNH4 - AupN*NH4/(NO3+NH4) + AexudN + SUM(ZexN)  )*dTd, 0.0)          
       write(6,*) istep,"NH4:",f(i,j,k,iNH4),RNH4*dTd,- AupN*NH4/(NO3+NH4)*dTd,AexudN*dTd,SUM(ZexN)*dTd
!----------------------------
!-Silica: (mmol-Si/m3)
!----------------------------
...
!--------------------------------------
      ...
      write(6,*) "PO4",istep,f(i,j,k,iPO4),RPO4*dTd,-AupP*dTd,AexudP*dTd,SUM(ZexP)*dTd
      !write(6,*) istep,f(i,j,k,iPO4),SUM(Z(:)*ZQp(:)),A_k(1,1)*Qc(isp)
!---------------------------------------------------------
!-DIC: Dissolved Inorganic Carbon (mmol-C/m3)
!---------------------------------------------------------
       ff(i,j,k,iDIC) = AMAX1(f(i,j,k,iDIC)                            &
       &  + ( RDIC - PrimProd + ArespC  + ZrespC )*dTd, 0.0)  
       write(6,*) "C in DIC, istep, DIC, RDIC, PrimProd, Aresp, Zresp"
       write(6,*) "C in DIC", istep, f(i,j,k,iDIC), RDIC*dTd , -PrimProd*dTd , ArespC*dTd  , ZrespC*dTd,      &
       & "Tot=",( RDIC - PrimProd + ArespC  + ZrespC)*dTd
!       write(6,*) ff(i,j,k,iDIC),f(i,j,k,iDIC)
...
! Let Oxygen go negative:
       ff(i,j,k,iO2) = f(i,j,k,iO2) + (RO2 + PrimProd - ArespC - ZrespC)*dTd
...
!-----------------------------------------
       ...
       !write(6,*) "f,ROM1_A,OM1_CA,dt",ff(i,j,k,iOM1_A),ROM1_A,OM1_CA,( (ROM1_A) + OM1_CA)*dTd
       !write(6,*) "PO4 in OM1A=f(OM1A)/s_x1A,OM1A,s_x1A",ff(i,j,k,iOM1_A)/s_x1A(i,j,k),ff(i,j,k,iOM1_A),s_x1A(i,j,k) 
...
!---------------------------------------------
       ...
       !write(6,*) "f,ROM2_A,OM2_CA,dt",ff(i,j,k,iOM2_A),ROM2_A,OM2_CA,( (ROM2_A) + OM2_CA)*dTd
       !write(6,*) "PO4 in OM2A=f(OM2A)/s_x2A",ff(i,j,k,iOM2_A)/s_x2A(i,j,k),ff(i,j,k,iOM2_A),s_x2A(i,j,k)
...
!------------------------------------------
       ...
       write(6,*) "f,ROM1_Z,OM1_CZ,Tot*dt",ff(i,j,k,iOM1_Z),ROM1_Z,OM1_CZ,( ROM1_Z + OM1_CZ)*dTd
       !write(6,*) "PO4 in OM1Z=f(OM1Z)/s_x1Z,OM1Z,s_x1Z",ff(i,j,k,iOM1_Z)/s_x1Z(i,j,k),ff(i,j,k,iOM1_Z),s_x1Z(i,j,k)
...
!-----------------------------------------------
       ...
       write(6,*) "f,ROM2_Z,OM2_CZ,Tot*dt",ff(i,j,k,iOM2_Z),ROM2_Z,OM2_CZ,( ROM2_Z + OM2_CZ)*dTd
       !write(6,*) "PO4 in OM2Z=f(OM2Z)/s_x2Z,OM2Z,s_x2Z",ff(i,j,k,iOM2_Z)/s_x2Z(i,j,k),ff(i,j,k,iOM2_Z),s_x2Z(i,j,k)
       write(6,*) "Total OM_Z",ff(i,j,k,iOM1_Z)+ff(i,j,k,iOM2_Z)
!---------------------------------------------------------------------
!-------------------------------------------
...
!-------------------------------------------
       ...
       &   + ( ROM1_BC )*dTd, 0.0)
       write(6,*) "f,ROM1_BC*dt",f(i,j,k,iOM1_BC),ROM1_BC*dTd
!---------------------------------------------------------------------
!------------------------------------------------
...
!------------------------------------------------
       ...
       &   + ( ROM2_BC )*dTd, 0.0)
       write(6,*) "f,ROM2_BC*dt",f(i,j,k,iOM2_BC),ROM2_BC*dTd
       write(6,*) "OMBC_tot",(ROM1_BC+ROM2_BC)*dTd
       write(6,*)
!---------------------------------------------------------------------
!----------------------------
!-CDOM: (ppb) 
