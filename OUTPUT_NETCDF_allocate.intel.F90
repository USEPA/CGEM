...
Subroutine OUTPUT_NETCDF_allocate
  ...
  ALLOCATE(character(len=6)::VARIABLE_NAMES(nf))
   counter = 0
 if(nospA.le.9) then
  do i=1,nospA
    counter = counter + 1
 ...
 elseif(nospZ.le.99) then
  ...
  ALLOCATE(character(len=100)::VARIABLE_DESCRIPTIONS(nf))
   counter = 0
  do i=1,nospA
    counter = counter + 1
  ...
  ALLOCATE(character(len=100)::VARIABLE_UNITS(nf))
  counter = 0
  do i=1,nospA
    counter = counter + 1
  ...
  ALLOCATE(character(len=19)::EXTRA_VARIABLE_NAMES(EXTRA_VARIABLES))
 ...
 elseif(nospA.le.99) then
  ...
  enddo
    ...
    EXTRA_VARIABLE_NAMES(counter+7) = 's_x2Z' 
    EXTRA_VARIABLE_NAMES(counter+8) = 's_y1Z' 
 ...
 elseif(nospA.le.99) then
  ...
  ALLOCATE(character(len=100)::EXTRA_VARIABLE_DESCRIPTIONS(EXTRA_VARIABLES))
 ...
 if(nospA.le.9) then
  ...
  ALLOCATE(character(len=100)::EXTRA_VARIABLE_UNITS(EXTRA_VARIABLES))
